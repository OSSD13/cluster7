        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-2">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow bug-card" data-team="{{ $bug['team'] ?? 'all' }}" data-name="{{ $bug['name'] ?? '' }}" data-sprint="{{ $bug['sprint_origin'] ?? $bug['sprint_number'] ?? '?' }}" data-id="{{ $bug['id'] }}">
                ...
                <div class="p-4 grid grid-cols-9">
                    ...
                    <!-- Edit Button -->
                    <div class="flex">
                        <button type="button" class="text-[#985E00] bg-[#FFC7B2] hover:bg-[#FFA954] focus:outline-none font-medium rounded-full px-2 py-2 text-center ms-1 h-8 w-8 edit-bug-btn" data-id="{{ $bug['id'] }}">
                        ...
                        <!-- Delete Button -->
                        <button type="button" class="text-red-700 bg-red-100 hover:bg-red-200 focus:outline-none font-medium rounded-full px-2 py-2 text-center ms-1 h-8 w-8 delete-bug-btn" data-id="{{ $bug['id'] }}">
...
<script>
    ...
    function deleteBug(bugId) {
        ...
        const loadingToast = showToast('Deleting bug...', 'info');
        ...
        .then(response => {
            if (!response.ok) {
                return response.json().then(data => {
                    throw new Error(data.error || 'Failed to delete the bug');
                });
            }
        ...
        .then(data => {
            ...
            // Remove the bug card from the UI
            const bugCard = document.querySelector(`.bug-card[data-id="${bugId}"]`);
            if (bugCard) {
                bugCard.remove();
                // Update bug counts
    ...
    function showToast(message, type = 'info') {
        ...
        toast.className = `fixed top-4 right-4 z-50 p-4 rounded-md shadow-md ${getToastColorClass(type)}`;
