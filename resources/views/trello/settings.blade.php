...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <!-- Connection Status -->
        @if($connectionStatus)
            <div class="p-4 border-b border-gray-200 {{ $connectionStatus['success'] ? 'bg-green-50' : 'bg-red-50' }}">
                <div class="flex items-center">
                    @if($connectionStatus['success'])
                        <div class="flex-shrink-0">
                            <div class="h-10 w-10 rounded-full bg-primary-100 flex items-center justify-center">
                                <span class="text-primary-600 font-semibold text-lg">{{ substr($connectionStatus['fullName'], 0, 1) }}</span>
                            </div>
                        </div>
                        <div class="ml-3">
                            <p class="text-green-700 font-medium">{{ $connectionStatus['message'] }}</p>
                            <p class="text-green-600 text-sm">Connected as: {{ $connectionStatus['fullName'] }} ({{ $connectionStatus['username'] }})</p>

                            @if(!empty($connectionStatus['boards']))
                                <div class="mt-2">
                                    <p class="text-xs font-semibold text-green-600">Recent boards:</p>
                                    <ul class="text-xs text-green-600 list-disc ml-5 mt-1">
                                        @foreach($connectionStatus['boards'] as $board)
                                            <li><a href="{{ $board['url'] }}" target="_blank" class="hover:underline">{{ $board['name'] }}</a></li>
                                        @endforeach
                                    </ul>
                                </div>
                            @endif
                    ...
                    @else
                        ...
                        <p class="text-red-700">{{ $connectionStatus['message'] }}</p>
                    @endif
                </div>
            </div>
        @endif
        ...
        <div class="p-6 bg-gray-50">
            <h3 class="text-lg font-medium text-gray-900 mb-4">How to get your Trello API credentials</h3>
    ...
    function handleSuccessResponse(data) {
        ...
        avatarContainer.innerHTML = `
            <div class="h-16 w-16 rounded-full bg-primary-100 flex items-center justify-center">
                <span class="text-primary-600 font-semibold text-2xl">${data.fullName.charAt(0)}</span>
            </div>
        `;
